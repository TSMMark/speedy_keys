  ...
  instanceMethods: {
    ...
    submitWordFor: function (playerId, value) {
      ...
      if (isValid && !winnerId && this.doesPlayerWin(index)) {
        ...
        $set["endedAt"] = this.props.endedAt;

        this.props.playersWPMs = this.calculatePlayersWPMs();
        $set["playersWPMs"] = this.props.playersWPMs;
      }
    ...
    calculatePlayersWPMs: function () {
      var gameSeconds = (this.props.endedAt - this.props.startedAt) / 1000
        , gameMinutes = gameSeconds / 60;

      return this.props.playersEnteredWords.map(function (words) {
        return words.length / gameMinutes;
      });
    },
    ...
    setCountdownSeconds: function (countdownSeconds) {
      ...
      if (countdownSeconds === 0) {
        ...
        $set["startedAt"] = this.props.startedAt;
      }
